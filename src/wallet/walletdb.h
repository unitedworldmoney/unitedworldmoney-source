...
public:
    ...
    bool WriteZUWMSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZUWMSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZUWMSeed_deprecated(uint256& seed);
    bool EraseZUWMSeed();
    bool EraseZUWMSeed_deprecated();

    bool WriteZUWMCount(const uint32_t& nCount);
    bool ReadZUWMCount(uint32_t& nCount);
