...
// The zUWM block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zUWM is a hash of the serial
...
{
    //Create an output returning the zUWM that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZUWMOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zUWM output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zUWM", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZUWMOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zUWM output", __func__);
...
{
    CzUWMTracker* zuwmTracker = pwallet->zuwmTracker.get();
    CMintMeta meta;
    if (!zuwmTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zuwmTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a UWM stake is the outpoint
