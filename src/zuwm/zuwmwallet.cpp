...
CzUWMWallet::CzUWMWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zuwm seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZUWMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZUWMSeed_deprecated()) {
                    LogPrintf("%s: Updated zUWM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zuwm seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zuwm wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzUWMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZUWMCount(nCountLastUsed);
    else if (!walletdb.ReadZUWMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzUWMWallet::Lock()
{
    seedMaster = 0;
}

void CzUWMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzUWMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZUWM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzUWMWallet::LoadMintPoolFromDB()
{
...
void CzUWMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzUWMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzUWMWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zuwmTracker->HasPubcoinHash(pMint.first)) {
...
bool CzUWMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZUWM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zuwmTracker which also adds to database
    pwalletMain->zuwmTracker->Add(dMint, true);
...
void CzUWMWallet::SeedToZUWM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzUWMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzUWMWallet::GenerateDeterministicZUWM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzUWMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzUWMWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzUWMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
