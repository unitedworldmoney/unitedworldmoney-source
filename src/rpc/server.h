...
/**
 * UnitedWorldMoney RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZuwmSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzuwmseed(const UniValue& params, bool fHelp);
extern UniValue getzuwmseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzuwm(const UniValue& params, bool fHelp);
extern UniValue dzuwmstate(const UniValue& params, bool fHelp);
