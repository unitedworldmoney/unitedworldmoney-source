...
#include "ui_zuwmcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zuwmTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zuwmTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zUWM.";
            else if (!mint.isSeedCorrect)
                strReason = "The zUWM seed used to mint this zUWM is not the same as currently hold in the wallet";
            else
