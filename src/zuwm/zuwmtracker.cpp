...
CzUWMTracker::CzUWMTracker(std::string strWalletFile)
{
...
CzUWMTracker::~CzUWMTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzUWMTracker::Init()
{
...
bool CzUWMTracker::Archive(CMintMeta& meta)
{
...
bool CzUWMTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzUWMTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzUWMTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzUWMTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzUWMTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzUWMTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzUWMTracker::GetSerialHashes()
{
...
CAmount CzUWMTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzUWMTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzUWMTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzUWMTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzUWMTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzUWMTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzUWMTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzUWMTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzUWMTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzUWMTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzUWMTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzUWMWallet* zUWMWallet)
{
    bool iszUWMWalletInitialized = (NULL != zUWMWallet);
    CMintMeta meta;
    ...
    if (! iszUWMWalletInitialized)
        zUWMWallet = new CzUWMWallet(strWalletFile);
    meta.isSeedCorrect = zUWMWallet->CheckSeed(dMint);
    if (! iszUWMWalletInitialized)
        delete zUWMWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzUWMTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzUWMTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzUWMTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzUWMTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzUWMTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzUWMTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzUWMWallet* zUWMWallet = new CzUWMWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zUWMWallet);
        }
        delete zUWMWallet;
        LogPrint("zero", "%s: added %d dzuwm from DB\n", __func__, listDeterministicDB.size());
    }
